...
const silencedMsgs =
    require('./config/silenced-msgs.json')['silenced-messages'];
...
mc.on('message', (message) => {
  ...
  if (config.extras.hypixelAutoGG.enabled &&
    ...
    setTimeout(() => {mc.chat("/ac gg")}, config.extras.hypixelAutoGG.interval);

    setTimeout(() => {mc.chat("/tip all")}, 5000);
  }

  if (!silence && message.replace(/\s/g, '').length) {
    log(message, true)

    if (message.includes("The game starts") ||
    ...
    else if ((message.includes(" joined the lobby!") &&
              message.includes("[MVP+")) ||
             message.includes("You are currently NICKED") ||
             (message.includes("[Mystery Box] ") &&
              message.includes(" found a")) ||
